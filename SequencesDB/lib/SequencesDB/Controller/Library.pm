...
sub result_list : Path('/result') : Args(0) {
    ...
    if ( $field_value && $advanced_search ) {
        $rs = $c->model('DB::Library')
          ->search_library( $field_value, $advanced_search, $search_options );
    }
...
sub process_request : Path('/new/library') : Args(0) {
    ...
    if ( !$file_id ) {
        ...
        if ($filename_validate) {
            ...
            $c->stash(
                ...
                path_validate       => $path,
                error15             => "Library ID "
                  . $sha2_validate->library_id
                  . " already exists! SHA-key: "
                  . $sha2_validate->sha2
    ...
    if ($file_id) {
        ...
        my $library = $c->model('DB::Library')
          ->find( { library_id => $file->library_library_id } );
        ...
        if ( $library->sha2 ne $sha2 ) {
            ...
            if ($sha2_aux) {
                $c->stash(
                    ...
                    error14 => "Library ID "
                      . $library->id
                      . " already exists! Search for SHA2-key: "
                      . $sha2
                      . " to obtain file info."
                );
                return;
...
sub edit : Path('/edit') : Args(0) {
    ...
    my $rs = $c->model('DB::Library')
      ->search_library( $file_id, "files_inner.file_id", "equal" );
    my $field = $rs->first;
